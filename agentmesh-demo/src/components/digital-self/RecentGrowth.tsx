  ...
  return (
    <section className="card-base grid min-h-[320px] min-w-0 grid-cols-[minmax(0,1fr)] grid-rows-[56px_1fr_40px] p-5" aria-labelledby="growth-heading">
      <header className="flex h-[56px] items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0 pt-0.5 text-left">
            ...
            ) : <p className="mt-1 text-[12px] leading-snug text-slate-500">知识、方法与协作经验</p>}
          </div>
        </div>
      ...
      <div className="divide-y divide-white/[0.06]">
        {model.loading ? <p role="status" className="py-5 text-sm text-slate-500">正在读取成长记录…</p> : null}
        {model.error ? <p role="alert" className="py-5 text-sm text-rose">{model.error}</p> : null}
        {!model.loading && !model.error && model.data.categories.length === 0 ? <p className="py-5 text-sm text-slate-500">尚无可展示的成长记录。</p> : null}
          ...
          return (
            <div key={item.label.value} className="flex min-h-[60px] items-center gap-3 py-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 truncate text-[12px] text-slate-500">{item.value.value}</p>
              </div>
            </div>
          )
        })}
