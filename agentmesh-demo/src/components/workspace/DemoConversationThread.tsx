  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <div className="max-w-[90%] rounded-[14px] rounded-tr-sm bg-surface-3 px-4 py-3 text-[14px] leading-relaxed text-slate-100">
      ...
      <div className="space-y-4">
        <div className="flex gap-2.5 text-[12.5px] leading-relaxed">
          <Brain className="mt-[3px] h-4 w-4 shrink-0 text-slate-500" />
          <div className="min-w-0 flex-1">
            <div className="font-medium text-slate-300">项目理解</div>
            <p className="mt-0.5 text-slate-500">家电设计师 · 618 会场改版 · 首屏结构决策 · 基于本人及团队已授权工作知识分析</p>
          </div>
        </div>
        ...
        <div>
          <button type="button" onClick={() => setAnalysisOpen((open) => !open)} className="flex items-center gap-1.5 text-left">
            ...
            <ChevronDown className={cn('h-3.5 w-3.5 text-slate-500 transition-transform', analysisOpen && 'rotate-180')} />
          </button>
          ...
          {analysisOpen ? (
            ...
            <div className="ml-[26px] mt-1 space-y-1 text-[12px] leading-relaxed text-slate-500">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 truncate text-[12.5px] text-slate-500">项目目标、历史经验、核心问题、设计原则、推荐方向和数据依据</p>
          </div>
        ...
        <div className="space-y-2.5">
          <p className="text-[12px] leading-relaxed text-slate-500">本次引用的团队经验和资料来源已记录在 Brief 中。</p>
          <div className="flex flex-col items-start gap-2">
            {FOLLOW_UPS.map((followUp) => (
              <button
                ...
                className="w-fit rounded-[10px] border border-white/[0.08] bg-surface-1 px-3 py-2 text-left text-[12.5px] text-slate-400 transition-colors hover:border-white/[0.18] hover:bg-surface-2 hover:text-slate-200"
              >
                {followUp}
              </button>
            ))}
