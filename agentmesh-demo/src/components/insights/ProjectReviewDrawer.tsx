  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {opportunity ? (
          <section>
            <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">复盘机会</h3>
        ...
        {evidence ? (
          <section className="border-t border-white/[0.06] pt-5">
            <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">参考证据检查</h3>
            <ul className="mt-3 space-y-2">
              {evidence.value.map((item) => (
                <li key={item.label} className="flex items-start gap-3 rounded-[10px] bg-surface-1 px-3.5 py-3">
                  {item.ready ? (
        ...
        {flow ? (
          <section className="border-t border-white/[0.06] pt-5">
            <h3 className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">知识形成条件</h3>
            <p className="mt-2 text-[13px] leading-6 text-slate-400">{flow.value.missingHint}</p>
            <div className="mt-3 rounded-[10px] bg-remind/[0.06] px-3.5 py-3">
        ...
        {!opportunity && !evidence && !flow ? (
          <p className="text-sm leading-6 text-slate-500">当前没有可安全展示的复盘参考内容。</p>
        ) : null}
      </div>
    </Drawer>
  )
