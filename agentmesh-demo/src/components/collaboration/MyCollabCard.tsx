    ...
    <article data-task-id={record.id.value} className="card-base flex min-h-[290px] flex-col p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-medium uppercase tracking-wide text-slate-500">
      ...
      <div className="mt-4 grid gap-3 rounded-[12px] bg-surface-2 p-4 text-xs sm:grid-cols-2">
        <div>
          <div className="text-slate-500">当前负责人</div>
          <div className="mt-1 font-medium text-slate-200">{record.owner.value ?? '尚未分配'}</div>
        </div>
        <div>
          <div className="text-slate-500">完成条件</div>
      ...
      <div className="mt-4">
        <div className="text-[10.5px] font-medium uppercase tracking-wide text-slate-600">本次获得</div>
          ...
          <ul className="mt-2 space-y-2">
            {record.contributions.value.slice(0, 3).map((contribution, index) => (
              <li key={`${contribution.actor}-${contribution.kind}-${index}`} className="flex items-start gap-2 text-[12px] leading-relaxed text-slate-300">
                ...
                <span><span className="text-slate-500">{contribution.actor}：</span>{contribution.detail}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="mt-2 text-[12px] text-slate-500">任务已建立，等待协作方补充内容。</p>
        )}
      </div>
              ...
              <span className="flex -space-x-2">
                {participants.slice(0, 4).map((participant) => (
                  <Avatar
                    ...
                    className="border-2 border-surface-1 text-[10px]"
                  />
                ))}
              </span>
              <span className="text-[11px] text-slate-500">{participants.length} 位参与者</span>
            </summary>
            <div className="absolute bottom-full left-0 z-20 mb-2 w-[min(320px,80vw)] rounded-[12px] border border-white/[0.08] bg-surface-2 p-4 shadow-pop">
                  ...
                  return (
                    <div key={participant.name} className="flex gap-3 border-t border-white/[0.05] pt-3 first:border-0 first:pt-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 text-[10.5px] text-slate-500">{RELATIONSHIP_LABELS[participant.relationship]}</div>
