...
export function AgentModule({ context }: AgentModuleProps) {
  ...
  const configurableAgents = resource.agents.data?.items.filter(
    (agent) => agent.agent_type !== 'personal' || agent.owner_user_id === context.userId,
  ) ?? []
  const [requestedAgentId, setRequestedAgentId] = useState<string | null>(null)
  const selectedAgentId = configurableAgents.some((agent) => agent.id === requestedAgentId)
    ? requestedAgentId
    : configurableAgents[0]?.id ?? null
  const agentTools = useAgentToolsAdmin(context, selectedAgentId, true)
  const [selectedToolIds, setSelectedToolIds] = useState<string[]>(
    agentTools.isSuccess ? agentTools.data.items.map((tool) => tool.id) : [],
  )
  const [initializedAgentId, setInitializedAgentId] = useState<string | null>(
    agentTools.isSuccess ? selectedAgentId : null,
  )
  const selectedAgent = configurableAgents.find((agent) => agent.id === selectedAgentId)
  ...
  return (
    <Card padding="lg">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">查看公共 Agent 与你的个人 Agent，并配置基础模型和工具。</p>
      </div>
      {resource.agents.isLoading ? <p className="mt-5 text-sm text-slate-400">正在加载 Agent…</p> : null}
      {resource.agents.isError ? <p role="alert" className="mt-5 text-sm text-rose">{message(resource.agents.error)}</p> : null}
      {!resource.agents.isLoading && !resource.agents.isError && configurableAgents.length === 0 ? <p className="mt-5 text-sm text-slate-400">当前没有可配置的 Agent。</p> : null}
      {configurableAgents.length > 0 ? (
        <div className="mt-5 grid gap-5 lg:grid-cols-[280px_1fr]">
          <div className="space-y-2" role="list" aria-label="可配置 Agent">
            {configurableAgents.map((agent) => (
              <button
                key={agent.id}
                type="button"
                onClick={() => setRequestedAgentId(agent.id ?? null)}
                className={`w-full rounded-soft border px-4 py-3 text-left transition-colors ${agent.id === selectedAgentId ? 'border-mint-400/35 bg-mint-400/[0.08]' : 'border-white/[0.06] bg-surface-2 hover:border-white/[0.12]'}`}
              >
                ...
                <span className="mt-2 flex gap-2"><Badge tone={agent.agent_type === 'personal' ? 'mint' : 'neutral'}>{agent.agent_type === 'personal' ? '个人' : '公共'}</Badge><Badge>{agent.status}</Badge><Badge>{agent.runtime_status}</Badge></span>
              </button>
            ))}
          </div>
          {selectedAgent ? (
            <div className="space-y-5 rounded-soft border border-white/[0.06] bg-surface-2 p-5">
              <div>
                ...
                <p className="mt-2 text-xs text-slate-400">当前任务：{selectedAgent.current_task_title ?? '无'}</p>
              </div>
              <label className="block text-sm text-slate-300">
                ...
                <select
                  ...
                  className="mt-2 h-10 w-full rounded-soft border border-white/[0.08] bg-surface-1 px-3 text-white outline-none"
                >
              ...
              <fieldset disabled={toolGrantsUnavailable || resource.updateTools.isPending}>
                ...
                {agentTools.isFetching || resource.tools.isFetching ? <p className="mt-2 text-sm text-slate-400">正在加载当前工具授权…</p> : null}
                ...
                {!agentTools.isFetching && !resource.tools.isFetching && !agentTools.isError && resource.tools.data && resource.tools.data.items.length > 0 ? (
                  <div className="mt-2 grid gap-2 sm:grid-cols-2">
                    {resource.tools.data.items.map((tool) => (
                      <label key={tool.id} className="flex items-start gap-3 rounded-soft border border-white/[0.06] bg-surface-1 p-3 text-sm text-slate-300">
                        ...
                        <span><span className="block font-medium text-slate-100">{tool.name}</span><span className="mt-1 block text-xs text-slate-400">{tool.provider} · {tool.risk_level}</span></span>
                      </label>
                    ))}
                  </div>
                ) : null}
                {!agentTools.isFetching && !resource.tools.isFetching && !agentTools.isError && resource.tools.data?.items.length === 0 ? <p className="mt-2 text-sm text-slate-400">当前没有可用工具。</p> : null}
                <Button
                  className="mt-3"
                  variant="secondary"
