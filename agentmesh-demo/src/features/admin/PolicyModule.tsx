...
export function PolicyModule({ context, canViewPermissions, canViewRisk }: PolicyModuleProps) {
  ...
  return (
    <div className="space-y-6">
      {canViewPermissions ? (
        <Card padding="lg">
          <div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-mint-300" /><div><h2 className="text-lg font-semibold text-white">权限策略</h2><p className="mt-1 text-sm text-slate-500">只读显示当前角色规则。</p></div></div>
          {resource.permissionPolicies.isLoading ? <p className="mt-5 text-sm text-slate-500">正在加载权限策略…</p> : null}
          ...
          {resource.permissionPolicies.data && resource.permissionPolicies.data.items.length === 0 ? <p className="mt-5 text-sm text-slate-500">没有权限策略。</p> : null}
          {resource.permissionPolicies.data && resource.permissionPolicies.data.items.length > 0 ? (
            <ul className="mt-5 space-y-2">
              {resource.permissionPolicies.data.items.map((rule) => (
                <li key={rule.id} className="flex flex-wrap items-center gap-3 rounded-[10px] border border-white/[0.06] bg-surface-2 px-4 py-3 text-sm">
                  <span className="font-medium text-slate-100">{rule.action}</span><Badge>{rule.role}</Badge><Badge tone={rule.effect === 'allow' ? 'mint' : 'rose'}>{rule.effect}</Badge><span className="min-w-0 flex-1 text-slate-500">{rule.description || '无说明'}</span><span className="text-xs text-slate-500">{rule.enabled ? 'enabled' : 'disabled'}</span>
                </li>
              ))}
            </ul>
          ) : null}
      ...
      {canViewRisk ? (
        <Card padding="lg">
          <div><h2 className="text-lg font-semibold text-white">风险策略</h2><p className="mt-1 text-sm text-slate-500">只读显示判定信号和决策。</p></div>
          {resource.riskPolicies.isLoading ? <p className="mt-5 text-sm text-slate-500">正在加载风险策略…</p> : null}
          {resource.riskPolicies.isError ? <p role="alert" className="mt-5 text-sm text-rose">{message(resource.riskPolicies.error)}</p> : null}
          {resource.riskPolicies.data && resource.riskPolicies.data.items.length === 0 ? <p className="mt-5 text-sm text-slate-500">没有风险策略。</p> : null}
