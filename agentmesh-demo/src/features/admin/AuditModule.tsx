...
export function AuditModule({ context }: AuditModuleProps) {
  ...
  return (
    <Card padding="lg">
      <div className="flex items-center gap-3"><ScrollText className="h-5 w-5 text-mint-300" /><div><h2 className="text-lg font-semibold text-white">审计日志</h2><p className="mt-1 text-sm text-slate-400">服务端返回的最近审计事件。</p></div></div>
      <form onSubmit={applyFilters} className="mt-5 flex flex-wrap items-end gap-3">
        <label className="min-w-48 flex-1 text-sm text-slate-300">
          动作
          <input aria-label="审计动作" value={action} onChange={(event) => setAction(event.target.value)} placeholder="例如 create_user" className="mt-2 h-10 w-full rounded-soft border border-white/[0.08] bg-surface-1 px-3 text-white outline-none focus:border-mint-400/50" />
        </label>
        <label className="min-w-48 flex-1 text-sm text-slate-300">
          对象类型
          <input aria-label="审计对象类型" value={targetType} onChange={(event) => setTargetType(event.target.value)} placeholder="例如 user" className="mt-2 h-10 w-full rounded-soft border border-white/[0.08] bg-surface-1 px-3 text-white outline-none focus:border-mint-400/50" />
        </label>
