    ...
    return (
      <main className="flex min-h-full items-center justify-center bg-canvas px-5 py-10">
        <section className="w-full max-w-md text-center" aria-live="polite">
          ...
          {logoutError ? (
            <p role="alert" className="mt-4 rounded-soft bg-rose/10 px-4 py-3 text-sm leading-6 text-rose">
              {logoutError}
            </p>
          ) : null}
    ...
    <main className="grid min-h-full bg-canvas lg:grid-cols-[minmax(0,1.15fr)_minmax(420px,0.85fr)]">
      <section className="relative hidden overflow-hidden border-r border-white/[0.06] bg-base px-12 py-14 lg:flex lg:flex-col lg:justify-between">
        <div className="flex items-center gap-3">
          <span className="flex h-11 w-11 items-center justify-center rounded-soft bg-mint-400/12 text-mint-200">
            <Network className="h-6 w-6" aria-hidden="true" />
          </span>
          <div>
            <p className="text-base font-semibold text-slate-100">AgentMesh</p>
            <p className="text-xs text-slate-400">团队智能协作空间</p>
          </div>
        </div>
        <div className="max-w-xl pb-10">
          <p className="mb-5 text-xs font-semibold uppercase tracking-[0.18em] text-mint-200">Internal pilot</p>
        ...
        <p className="text-xs text-slate-400">同源会话，不在浏览器存储访问令牌</p>
      </section>

      <section className="flex min-h-full items-center justify-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-[420px]">
          <div className="mb-10 flex items-center gap-3 lg:hidden">
            <span className="flex h-10 w-10 items-center justify-center rounded-soft bg-mint-400/12 text-mint-300">
          ...
          <div className="mb-8">
            <span className="mb-5 flex h-10 w-10 items-center justify-center rounded-soft bg-white/[0.04] text-slate-300">
              <LockKeyhole className="h-5 w-5" aria-hidden="true" />
            </span>
          ...
          <section aria-labelledby="demo-roles-title">
            <div className="flex items-baseline justify-between gap-4">
              ...
              <p className="text-xs text-slate-400">一键进入预设角色</p>
            </div>
              ...
              {DEMO_ROLES.map((role) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-[88px] touch-manipulation rounded-soft border px-2.5 py-3 text-center transition-[border-color,background-color,transform] duration-150 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint-400/50 disabled:cursor-not-allowed disabled:opacity-45 ${
                      active
                  ...
                  >
                    ...
                    <span className="mt-1 block text-[11px] leading-4 text-slate-400">{role.detail}</span>
                  </button>
                )
              })}
            </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit} noValidate>
            <div>
              ...
              <input
                ...
                className="h-12 w-full rounded-soft border border-white/[0.1] bg-surface-1 px-4 text-[15px] text-slate-100 outline-none transition-[border-color,box-shadow] placeholder:text-slate-500 focus:border-mint-400/70 focus:ring-2 focus:ring-mint-400/15"
                placeholder="输入账号"
              />
            </div>
            <div>
            ...
            {error ? (
              <p role="alert" className="rounded-soft bg-rose/10 px-4 py-3 text-sm leading-6 text-rose">
                {error}
              </p>
            ) : null}
