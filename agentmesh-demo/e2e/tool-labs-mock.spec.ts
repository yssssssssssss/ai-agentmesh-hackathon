...
test('opens the aesthetic mock tool above the composer and preserves the chat draft', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '美学量化 Mock', exact: true })).toBeVisible()
  ...
  await expect(dialog.getByText('前端固定样例，不代表真实图片分析')).toBeVisible()
  ...
  await dialog.getByRole('button', { name: '生成演示结果' }).click()
  await expect(dialog.getByText('综合分 84')).toBeVisible()
  ...
  await expect(dialog.getByText('主视觉形成第一注意力峰值。')).toHaveCount(0)
...
test('produces a deterministic experience-model recommendation without backend requests', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '体验模型 Mock', exact: true })).toBeVisible()
