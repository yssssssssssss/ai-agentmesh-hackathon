...
def test_memory_review_decision_is_rejected_before_json_parsing_over_64_kib() -> None:
    client = TestClient(app)
    response = client.post(
        "/api/memory-reviews/review-missing/decisions",
        content=b"x" * (64 * 1024 + 1),
        headers={"content-type": "application/json"},
    )
...
def test_memory_governance_mutation_is_rejected_before_json_parsing_over_64_kib() -> None:
    client = TestClient(app)
    response = client.post(
        "/api/memory/memory-missing/revisions",
        content=b"x" * (64 * 1024 + 1),
        headers={"content-type": "application/json"},
    )
