...
def test_v2_identifiers_remain_disjoint_and_no_source_alias_is_registered() -> None:
    ...
    assert {"research-v3"} | V3_PERSISTED_SCHEMA_IDENTITIES == V3_PERSISTED_RESOURCE_IDENTITIES
    assert {"research-workbench-aggregate-v1"} == V3_WEB_SCHEMA_IDENTITIES
...
def test_gate2_defaults_keep_research_v3_dormant(tmp_path, monkeypatch) -> None:  # noqa: ANN001
    monkeypatch.delenv("AGENTMESH_SKILL_ORCHESTRATION", raising=False)
    monkeypatch.delenv("AGENTMESH_RESEARCH_PREVIEW_ALLOWLIST", raising=False)
    repository = SQLiteStore(tmp_path / "gate2-defaults.sqlite3")
    control = repository.get_research_writer_control()

    assert skill_orchestration_mode() == SkillOrchestrationMode.OFF
    assert research_preview_allowlist() == frozenset()
    assert control.active_generation.value == "research-v2"
    assert control.generation_epoch == 1
    assert not any(
        run.orchestration_version == "research-v3"
        for run in repository.list_agent_runs()
    )
