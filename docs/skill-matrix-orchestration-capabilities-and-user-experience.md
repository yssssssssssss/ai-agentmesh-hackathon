...
| 权限与就绪过滤 | 召回前检查 Skill 启用状态、绑定、Wiki 语料和项目权限，并把外部工具完整度单独标记为 `complete / tool_limited / unavailable` | 用户能区分“已接入可启动”和“依赖工具已完整接通”，不会把目录接入误解为外部工具已授权 |
...
| 执行前确认 | 多 Skill 计划进入确认页；用户只能增删候选范围内的可选节点、调整合法顺序，必需节点不可删除 | 用户在消耗模型和运行工具前能够看懂并控制实际执行内容 |
| 用户级工具授权 | Skill 内的只读 Tool 继承当前用户个人 Agent 的持久化 Tool Grant；只有外部写入、不可逆操作或命中高风险策略时才逐次确认 | 用户为 Agent 配置一次只读能力后，Skill 可直接使用，不会在执行途中重复询问 |
...
| SSE 实时进度 | 提供计划、节点、高风险确认、综合、完成和失败事件流 | 用户能实时看到“规划中、等待确认、正在分析、等待高风险操作、部分完成”等状态 |
...
- `$` 选择器在 Agent Runtime v2 和 Wiki 语料就绪时提供 95 个入口：84 个设计领域 Skill 和 11 个平台能力。
- 84 个领域 Skill 全部按设计生命周期归类：设计前 17 个、设计中 26 个、设计后 41 个；11 个平台能力单独归入“工具”。
- 84 个领域 Skill 中，57 个当前为完整执行状态，27 个因声明的外部工具尚无 AgentMesh 适配器而标记为“已接入 · 工具待接通”。后者仍可读取 Wiki 资源并显式启动，但涉及缺失工具的步骤会受限。
- `$` 选择器只展示当前可调用项，并使用两级菜单：一级为设计前、设计中、设计后、未分类和工具，桌面 hover、键盘或移动端点击后，二级才展示具体 Skill。
- 两级菜单使用固定浮层高度，分类切换不改变锚点位置；二级列表拥有独立滚动容器。
- 列表描述使用不超过 100 字的 `display_description`；检索使用公开描述中的正向能力段，明确标为 `Boundary` 的反例不会写入 FTS 或 Embedding。
- 展示 Skill 是否已绑定、是否具备规划条件、执行完整度、缺失工具以及不可用原因。
...
- 用户信息不足时，在规划阶段提示“需要补充”，或在仍可产出时标记“部分完成”，不归类为用户导致的执行失败。
- `执行失败` 仅表示运行条件未满足，例如 Provider 断连、权限在执行前被撤销、超时或输出未通过契约校验。
- 每个失败节点展示可读原因、系统已采取的动作和下一步建议；原始错误码默认折叠为技术诊断。
- 只读或草稿节点遇到临时连接错误时最多自动重试一次；写入、权限、安全和确定性校验错误不自动重放。
...
6. 用户可删除可选节点或调整合法顺序。
7. 确认后执行；独立节点并行运行。
8. 已授权的只读工具直接继承用户的个人 Agent 权限；只有外部写入、不可逆操作或命中高风险策略时才暂停确认。
9. 被暂停的高风险操作确认后从原节点恢复，而不是重新执行整份计划。
...
- 84 个唯一领域 Skill、10 个编排画像、11 个 Legacy 画像检查：0 错误、0 警告。
...
- 自动编排仍只使用 10 个已治理的 Wiki 领域 Skill；其余 74 个已接入但仅允许显式调用。
- 目录接入不等于外部工具已接通：27 个 `tool_limited` Skill 在补齐受治理的适配器前不能承诺完整执行；系统不会为此自动开放 Bash、文件写入或 Zero/JoySpace 权限。
