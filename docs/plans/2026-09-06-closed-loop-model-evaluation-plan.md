...
- 日期：2026-09-06
- 状态：已批准；D0、D1、R1 与 R2 已完成，R3 至 R4 等待维护者根据 R2 报告逐批批准
...
R1 不设置预先的合同通过率门槛。实际运行已完成 24/24，机器合同通过 24/24；成功任务实测 288,914 Token，预检和失败尝试保守预留 60,000 Token，预算口径共使用 348,914/500,000 Token。T04 和 T05 使用真实模型完成三阶段 planning、evidence analysis 和 synthesis，但不重复执行生产 durable DeepSearch 状态机；该状态机继续由 D1 与专项测试覆盖。维护者根据该报告决定是否运行 R2。

### 8.4 R2：24 个缺失输入案例

R2 对应 V1。案例在明确缺口、完成澄清或生成受限输出后停止，不创建无意义的 Review 或 Memory。实际运行已完成 24/24，机器合同通过 24/24，实测使用 167,658/250,000 Token；24 个案例均进入真实模型，当前 Input Preflight 没有节省调用。
...
eval/closed_loop/real_runner.py
eval/run_closed_loop_eval.py
tests/test_closed_loop_eval.py
```

预计涉及 7 个主要文件，不新增产品 API、数据库表、环境变量或前端页面。
...
| R1 | 已形成 24 个 V0 的 Token、合同通过率和失败报告；预算口径使用 348,914 Token | 已强制停止，等待维护者决定 R2 预算与阈值 |
| R2 | 已形成 24 个 V1 的缺失输入处理报告；实测使用 167,658 Token | 已强制停止，等待维护者决定 R3 预算与阈值 |
