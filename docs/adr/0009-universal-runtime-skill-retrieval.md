...
Universal Standard execution is a separate code capability from Preview. It remains unavailable until the execution contract and external release gates are satisfied; creation, approval, and executor entry all fail closed when it is unavailable. Native, MCP, and Skill-resource Tool dispatches append a durable claim inside the shared short quiesce permit before provider I/O and one terminal outcome afterward; the permit deliberately does not span external I/O, while pre-quiesce claims may only settle or be reconciled conservatively.
