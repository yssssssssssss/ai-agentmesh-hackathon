...
metadata:
  ...
  source: 2C-DesignWiki/jd-design-system-md-v16/product-architecture/platform-transaction/交易链路/购物车/SKILL.md
  agentmesh-wiki-import: 'true'
  agentmesh-stage: during_design
  agentmesh-activation: explicit_only
  short-description: 平台交易设计部购物车场域入口，按输入校验和四步方法路由购物车设计任务。
disable-model-invocation: true
---
# SKILL —— 购物车 场域路由

> 本文件是「购物车」场域的入口路由。上层总路由见 `../../_共享底座/SKILL.md`。
...
5. **落设计** → `4-落实设计/` 调用 coding foundation(设计系统 `foundations/tokens|设计语言基础|视觉规范/`、组件 `foundations/基础组件库|业务组件库/` 与 `foundations/场域特性规范/购物链路/购物车/<组件>/bundle/`、资源 `foundations/assets/`;读法见 `../../_共享底座/coding-foundation读取指南.md`);本场域专属组件在 `4-落实设计/组件/`。
